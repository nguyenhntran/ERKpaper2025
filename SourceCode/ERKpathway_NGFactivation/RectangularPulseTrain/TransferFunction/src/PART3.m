...
%   The same transfer function as H_coll but now in class 'tf'. Save it:
 ...
 % BODE PLOT
    ...
    % bode mag plot
    subplot(2,1,1)
    opts1.Title.String = sprintf('Bode magnitude plot | Mean [NGF] input u$_1$ = %g (unitless)' , input);
    bodeplot(TF,opts1);
    
    % bode phase plot
    subplot(2,1,2)
    opts2.Title.String = sprintf('Bode phase plot | Mean NGF input u$_1$ = %g (unitless)' , input);
    bodeplot(TF,opts2);
...
% PHASOR PROJECTIONS
    ...
    % add global title
    sgtitle(sprintf('Phasor Trajectory | Mean [NGF] input u$_1$ = %g (unitless)', input), 'FontSize',11, 'interpreter','latex')
