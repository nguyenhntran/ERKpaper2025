...
%   The same transfer function as H_coll but now in class 'tf'. Save it:
 ...
 % BODE PLOT
    ...
    % bode mag plot
    subplot(2,1,1)
    opts1.Title.String = sprintf('Bode magnitude plot | Mean [EGF] input u$_1$ = %g (unitless)' , input); %<-- Don't need to mention case no. in title because there is only one case
    bodeplot(TF,opts1);
    
    % bode phase plot
    subplot(2,1,2)
    opts2.Title.String = sprintf('Bode phase plot | Mean [EGF] input u$_1$ = %g (unitless)' , input); %<-- Don't need to mention case no. in title because there is only one case
    bodeplot(TF,opts2);

    % save plot
    % exportgraphics(h,sprintf('../plots/BodePlot_Case%g.pdf', ind),'BackgroundColor','none')
    savefig(h,sprintf('../plots/BodePlot_Case%g.fig', ind),'compact');
    close(h)
...
% PHASOR PROJECTIONS
    ...
    % add global title
    sgtitle(sprintf('Phasor Trajectory | Mean [EGF] input u$_1$ = %g (unitless)', input), 'FontSize',11, 'interpreter','latex')

    % save plot
    % exportgraphics(h1,sprintf('../plots/Phasor_Case%g.pdf', ind),'BackgroundColor','none')
    savefig(h1,sprintf('../plots/Phasor_Case%g.fig', ind),'compact');
    close(h1)
