...
% |                                                                     |
% | Inputs:                                                             |
% | TransFuncCase7_ClassTf.mat                                          |  %<--- filename
% |                                                                     |
% | Outputs:                                                            |
% | Transformation_Case7.pdf                                            |  %<--- filename
% | OutputStats_Case7.txt                                               |  %<--- filename
%  ---------------------------------------------------------------------
...
tl = tiledlayout(2,1);
title(tl,'Input-output transformation: Low amplitude pulses at frequency 1/1380 s$^{-1}$', 'Interpreter','latex') %<--- title case number
...
amplitude = 1/12; %unitless concentration                                  %<--- pulse amplitude
...
load('../../TransferFunction/data/TransFuncCase7_ClassTf.mat')             %<--- filename
...
% Define output
ERKstar_equilibrium = 0.0698213995856706;                                  %<--- equilibrium output obtained from timecourse similation of nonlinear model
newexpansion(t) = ERKstar_equilibrium + newcosterms_sum + newsinterms_sum;

% Save output function
save('../data/OutputFourierExpansion_Case7.mat','newexpansion')            %<--- filename

% Plot output
nexttile
fplot(newexpansion,[0,150*60])
ylim([-0.05 0.3])                                                          %<--- y axis limits
...
exportgraphics(h1,'../plots/Transformation_Case7.pdf','BackgroundColor','none'); %<--- filename
close(h1);
...
% fileID = fopen('../data/OutputStats_Case7.txt','w');                       %<--- case number
% 
% % Print stats
% fprintf(fileID, "CASE 7 ERK* OUTPUT: \n\n");                               %<--- case number
