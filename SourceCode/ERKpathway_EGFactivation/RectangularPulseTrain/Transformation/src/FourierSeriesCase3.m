...
% |                                                                     |
% | Inputs:                                                             |
% | TransFuncCase3_ClassTf.mat                                          |  %<--- filename
% |                                                                     |
% | Outputs:                                                            |
% | Transformation_Case3.pdf                                            |  %<--- filename
% | OutputStats_Case3.txt                                               |  %<--- filename
%  ---------------------------------------------------------------------
...
tl = tiledlayout(2,1);
title(tl,'Input-output transformation: High amplitude pulses at frequency 1/1380 s$^{-1}$', 'Interpreter','latex') %<--- title case number
...
pauseduration = 20*60; %seconds                                            %<--- pulse pause duration
...
load('../../TransferFunction/data/TransFuncCase3_ClassTf.mat')             %<--- filename
...
% Define output
ERKstar_equilibrium = 0.240199766152701;                                   %<--- equilibrium output obtained from timecourse similation of nonlinear model
newexpansion(t) = ERKstar_equilibrium + newcosterms_sum + newsinterms_sum;

% Save output function
save('../data/OutputFourierExpansion_Case3.mat','newexpansion')            %<--- filename

% Plot output
nexttile
fplot(newexpansion,[0,150*60])
ylim([0.15 0.53])                                                            %<--- y axis limits
...
exportgraphics(h1,'../plots/Transformation_Case3.pdf','BackgroundColor','none'); %<--- filename
close(h1);
...
% fileID = fopen('../data/OutputStats_Case3.txt','w');                       %<--- case number
% 
% % Print stats
% fprintf(fileID, "CASE 3 ERK* OUTPUT: \n\n");                               %<--- case number
