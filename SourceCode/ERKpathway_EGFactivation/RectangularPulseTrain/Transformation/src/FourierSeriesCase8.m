...
% |                                                                     |
% | Inputs:                                                             |
% | TransFuncCase8_ClassTf.mat                                          |  %<--- filename
% |                                                                     |
% | Outputs:                                                            |
% | Transformation_Case8.pdf                                            |  %<--- filename
% | OutputStats_Case8.txt                                               |  %<--- filename
%  ---------------------------------------------------------------------
...
tl = tiledlayout(2,1);
title(tl,'Input-output transformation: Low amplitude pulses at frequency 1/3780 s$^{-1}$', 'Interpreter','latex') %<--- title case number
...
pauseduration = 60*60; %seconds                                            %<--- pulse pause duration
...
load('../../TransferFunction/data/TransFuncCase8_ClassTf.mat')             %<--- filename
...
% Define output
ERKstar_equilibrium = 0.0257056081572630;                                  %<--- equilibrium output obtained from timecourse similation of nonlinear model
newexpansion(t) = ERKstar_equilibrium + newcosterms_sum + newsinterms_sum;

% Save output function
save('../data/OutputFourierExpansion_Case8.mat','newexpansion')            %<--- filename
...
exportgraphics(h1,'../plots/Transformation_Case8.pdf','BackgroundColor','none'); %<--- filename
close(h1);
...
% fileID = fopen('../data/OutputStats_Case8.txt','w');                       %<--- case number
% 
% % Print stats
% fprintf(fileID, "CASE 8 ERK* OUTPUT: \n\n");                               %<--- case number
