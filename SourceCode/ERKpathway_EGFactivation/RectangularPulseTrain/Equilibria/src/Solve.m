...
% | DOES THIS FOR CONSTANT EGF INPUT =                                   |
% | 25*3/6 (ng/ml)                                                       |
% | 25*3/13 (ng/ml)                                                      |
% | 25*3/23 (ng/ml)                                                      |
% | 25*3/63 (ng/ml)                                                      |
...
% Define EGF input values (units ng/ml)
EGFvals = [25*3/6 , 25*3/13 , 25*3/23 , 25*3/63 , 1*3/6 , 1*3/13 , 1*3/23 , 1*3/63];
% Convert EGF input values to dimensionless u values used in Pertz2015
EGFvals = EGFvals / 12;
...
% For each case of input value in EGFvals
for ind = 1:length(EGFvals)
    ...
    h=figure();
    set(h,'units','centimeters','position',[0,0,21,28])
    tl = tiledlayout('flow');
    tl.TileSpacing = 'compact';
    tl.Padding = 'compact';
    title(tl, 'Pathway concentration timecourses') 
    subtitle(tl,sprintf('under constant [EGF] input u_1(t) = %g (unitless)', u1))
    xlabel(tl,'t (sec)')
    ...
    ylim([-0.1 1.1]);
    legend('x_1(t)', 'x_2(t)', 'sum', 'Location', 'southoutside', 'Orientation', 'horizontal');
    ...
    ylim([-0.1 1.1]);
    legend('x_3(t)', 'x_4(t)', 'sum', 'Location', 'southoutside', 'Orientation', 'horizontal');
    ...
    ylim([-0.1 1.1]);
    legend('x_5(t)', 'x_6(t)', 'sum', 'Location', 'southoutside', 'Orientation', 'horizontal');
    ...
    ylim([-0.1 1.1]);
    legend('x_7(t)', 'x_8(t)', 'sum', 'Location', 'southoutside', 'Orientation', 'horizontal');
    ...
    ylim([-0.1 1.1]);
    legend('x_9(t)', 'x_{10}(t)', 'sum', 'Location', 'southoutside', 'Orientation', 'horizontal');
    ...
    ylim([-0.1 1.1]);
    legend('x_{11}(t)', 'x_{12}(t)', 'sum', 'Location', 'southoutside', 'Orientation', 'horizontal');
    ...
    ylim([-0.1 1.1]);
    legend('x_{13}(t)', 'x_{14}(t)', 'sum', 'Location', 'southoutside', 'Orientation', 'horizontal');
    ...
    legend('x_{15}(t)', 'x_{16}(t)', 'sum', 'Location', 'southoutside', 'Orientation', 'horizontal');

    % Plot separate interactive figure for [R] and [R*] timecourses
    h0=figure();
    plot(t,x(:,1),'-.');
    hold on
    plot(t,x(:,2),'-.');
    ...
    title('x_1(t) = [R] and x_2(t) = [R*]');
    xlabel('t (sec)')
    ylabel('x_n(t) (unitless)')
    ylim([-0.1 1.1]);
    legend('x_1(t)', 'x_2(t)', 'sum', 'Location','best');

    % Plot separate interactive figure for [ERK] and [ERK*] timecourses
    h1=figure();
    plot(t,x(:,9),'-.');
    hold on
    plot(t,x(:,10),'-.');
    ...
    ylabel('x_n(t) (unitless)')
    ylim([-0.1 1.1]);
    legend('x_9(t)', 'x_{10}(t)', 'sum', 'Location','best');

% ------------------------------ SAVING -----------------------------------

    % Save first timecourse plot with all species
    savefig(h,sprintf('../plots/TimecourseCase%g.fig', ind),'compact');
    % Close figure h (we couldn't create h with visibility 'off' because that would make the saved fig files also have visibility off when opened later)
    close(h)

    % Save second interactive timecourse plot with ERK and ERK*
    savefig(h0,sprintf('../plots/TimecourseCase%g_R.fig', ind),'compact');
    % Close figure h0 (we couldn't create h0 with visibility 'off' because that would make the saved fig files also have visibility off when opened later)
    close(h0)

    % Save third interactive timecourse plot with ERK and ERK*
