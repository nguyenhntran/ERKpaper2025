...
% |                                                                     |
% | Inputs:                                                             |
% | TransFuncCase1_ClassTf.mat                                          |  %<--- filename
% |                                                                     |
% | Outputs:                                                            |
% | Transformation_Case1.pdf                                            |  %<--- filename
% | OutputStats_Case1.txt                                               |  %<--- filename
%  ---------------------------------------------------------------------
...
tl = tiledlayout(2,1);
title(tl,'Input-output transformation: High amplitude pulses at frequency 1/360 s$^{-1}$', 'Interpreter','latex') %<--- title case number
...
amplitude = 25/12; %unitless concentration                                 %<--- pulse amplitude
pulseduration = 3*60; %seconds
pauseduration = 3*60; %seconds                                             %<--- pulse pause duration
...
load('../../TransferFunction/data/TransFuncCase1_ClassTf.mat')             %<--- filename
...
% Define output
ERKstar_equilibrium = 0.157556940592814;                                   %<--- equilibrium output obtained from timecourse similation of nonlinear model
newexpansion(t) = ERKstar_equilibrium + newcosterms_sum + newsinterms_sum;

% Save output function
save('../data/OutputFourierExpansion_Case1.mat','newexpansion')            %<--- filename

% Plot output
nexttile
fplot(newexpansion,[0,150*60])                                             % !!!!!!!!!!!!!!!!!!!!!!!!!!!!
ylim([0.150 0.165])                                                        %<--- y axis limits
...
exportgraphics(h1,'../plots/Transformation_Case1.pdf','BackgroundColor','none'); %<--- filename
close(h1);
...
% fileID = fopen('../data/OutputStats_Case1.txt','w');                       %<--- case number
% 
% % Print stats
% fprintf(fileID, "CASE 1 ERK* OUTPUT: \n\n");                               %<--- case number
