...
for ind = 1:length(EGFvals)
    ...
    tl.Padding = 'compact';
    title(tl, 'Pathway concentration timecourses')                         %<-- Don't need to mention case no. in title because there is only one case
    subtitle(tl,sprintf('under constant [EGF] input u(t) = %g (unitless)', u1))
    xlabel(tl,'t (sec)')
    ...
    legend('x_{15}(t)', 'x_{16}(t)', 'Location', 'southoutside', 'Orientation', 'horizontal');
