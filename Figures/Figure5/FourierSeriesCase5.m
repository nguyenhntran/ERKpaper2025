...
% |                                                                     |
% | Inputs:                                                             |
% | TransFuncCase5_ClassTf.mat                                          |  %<--- filename
% |                                                                     |
% | Outputs:                                                            |
% | Transformation_Case5.pdf                                            |  %<--- filename
% | OutputStats_Case5.txt                                               |  %<--- filename
%  ---------------------------------------------------------------------
...
tl = tiledlayout(2,1);
title(tl,'Input-output transformation: Low amplitude pulses at frequency 1/360 s$^{-1}$', 'Interpreter','latex') %<--- title case number
...
load('../../TransferFunction/data/TransFuncCase5_ClassTf.mat')             %<--- filename
...
% Define output
ERKstar_equilibrium = 0.163674376034645;                                   %<--- equilibrium output obtained from timecourse similation of nonlinear model
newexpansion(t) = ERKstar_equilibrium + newcosterms_sum + newsinterms_sum;

% Save output function
save('../data/OutputFourierExpansion_Case5.mat','newexpansion')            %<--- filename

% Plot output
nexttile
fplot(newexpansion,[0,150*60])
ylim([0.12 0.2])                                                          %<--- y axis limits
...
exportgraphics(h1,'../plots/Transformation_Case5.pdf','BackgroundColor','none'); %<--- filename
close(h1);
...
% fileID = fopen('../data/OutputStats_Case5.txt','w');                       %<--- case number
% 
% % Print stats
% fprintf(fileID, "CASE 5 ERK* OUTPUT: \n\n");                               %<--- case number
