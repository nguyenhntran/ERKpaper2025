...
% |                                                                     |
% | Inputs:                                                             |
% | TransFuncCase6_ClassTf.mat                                          |  %<--- filename
% |                                                                     |
% | Outputs:                                                            |
% | Transformation_Case6.pdf                                            |  %<--- filename
% | OutputStats_Case6.txt                                               |  %<--- filename
%  ---------------------------------------------------------------------
...
tl = tiledlayout(2,1);
title(tl,'Input-output transformation: Low amplitude pulses at frequency 1/780 s$^{-1}$', 'Interpreter','latex') %<--- title case number
...
load('../../TransferFunction/data/TransFuncCase6_ClassTf.mat')             %<--- filename
...
% Define output
ERKstar_equilibrium = 0.106765758772716;                                   %<--- equilibrium output obtained from timecourse similation of nonlinear model
newexpansion(t) = ERKstar_equilibrium + newcosterms_sum + newsinterms_sum;

% Save output function
save('../data/OutputFourierExpansion_Case6.mat','newexpansion')            %<--- filename
...
exportgraphics(h1,'../plots/Transformation_Case6.pdf','BackgroundColor','none'); %<--- filename
close(h1);
...
% fileID = fopen('../data/OutputStats_Case6.txt','w');                       %<--- case number
% 
% % Print stats
% fprintf(fileID, "CASE 6 ERK* OUTPUT: \n\n");                               %<--- case number
